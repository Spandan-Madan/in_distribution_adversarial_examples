...
def get_save_names(category, instance, cam_pos, light_pos):
    cam_pos_string = "%.03f_%.03f_%.03f"%(cam_pos[0].item(),cam_pos[1].item(),cam_pos[2].item())
    light_pos_string = "fixed_lighting"
...
##### Placing lights at all octants #####
##### Placing them at fixed location + fixed object in world coordinates bakes the shadows into the object texture
##### so the only thing changing is the object viewpoint for a fixed texture completely removing the impact of lighting.

radius = 2.0
light_radius = 2*radius
light_val = np.sqrt(light_radius/3)
light_pos = []
xs = ys = zs = [light_val, -light_val]
for x in xs:
    for y in ys:
        for z in zs:
            light_point = torch.tensor([x,y,z]).float()
            light_pos.append(light_point)
