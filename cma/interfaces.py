...
"""
    ...
    def stop(self, stop_condition=False):
        if stop_condition:
            return True
    ...
    """
        ...
        with (EvalParallel2 or EvalParallel)(objective_fct,
            ...
            while not self.stop(stop_criterion) or citer < min_iterations:
                if stop_criterion:
                    if verb_disp:
                        print('Adversarial found, trying to stop.')
                    break
                ...
                ##### CUSTOM CODE FOR FUNCTION WITH TWO RETURN VALUES #####

                fitvals_ = eval_all(X, args=args)
                fitvals = [i[0] for i in fitvals_]
                fitvals_2 = [i[1] for i in fitvals_]
                for pred in fitvals_2:
                    if pred != correct_prediction:
                        if verb_disp:
                            print('Adversarial found, should stop now.')
                        stop_criterion = True
                        # self.stop()
                if verb_disp: # only print if the user asks for verbose display of predictions
                    print('Predictions are :%s'%fitvals_2)

                self.predictions = fitvals_2
                self.prediction_settings = X
