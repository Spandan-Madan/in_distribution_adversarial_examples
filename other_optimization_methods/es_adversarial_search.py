...
def objective(scene_params, optimized_params, neuron_num = None):
    ...
    inputs_ = (inputs - im_means)/im_stds
    outputs = softmax_layer(loaded_model(inputs_))
    loss = outputs[0][category_num].item()
    prediction = torch.argmax(outputs[0]).item()
    return loss, prediction



folder = '/om5/user/smadan/differentiable_graphics_ml/other_optimization_methods/es_adversarial_search/'
...
for epoch in range(1):
    ...
    for c in tqdm_file(population):
        try:
            score, prediction = objective(c, optimized_params, None)
            scores.append(score)
    ...
    for i in tqdm_file(selected):
        ...
        if scores[i] < best_eval:
            ...
            if predictions[i] not in [category_num, -1]:
                print('Found Adversarial Setting!')
                found_adv = True
                break
...
info_to_save['adversarial_setting'] = best
info_to_save['found_adversarial'] = found_adv

print('Adversarial Search Successful. Best loss=%s'%best_eval)
