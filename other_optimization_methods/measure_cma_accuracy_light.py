...
def evaluate_exp(exp_path, render = False, show = False):
    ...
    #ids = []
    #for i in len(range(all_predictions)):
    #    if all_predictions
    ...
    if len(ids[0]) != 0:
        found_adv = True
        adv_id = random.choice(ids)[0]
        adv_prediction = all_predictions[adv_id]
        predictions.append(adv_prediction)
        adv_prediction_settings = all_prediction_settings[adv_id]
        ...
        if varying_camera:
            ...
            num_lights = int(adv_prediction_settings.shape[0]/11)
            adv_scene_params[2]['all_light_positions'] = torch.tensor(adv_prediction_settings[:num_lights*3]).float().view(num_lights,-1)
            adv_scene_params[2]['all_light_look_ats'] = torch.tensor(adv_prediction_settings[num_lights*3:num_lights*6]).float().view(num_lights,-1)
            adv_scene_params[2]['all_light_intensities'] = torch.tensor(adv_prediction_settings[num_lights*6:num_lights*9]).float().view(num_lights,-1)
            adv_scene_params[2]['all_light_sizes'] = torch.tensor(adv_prediction_settings[num_lights*9:]).float().view(num_lights,-1)
...
folder = '/om5/user/smadan/differentiable_graphics_ml/other_optimization_methods/cma_adversarial_light/neurips_results/%s'%SAVE_FOLDER_NAME
...
for exp in tqdm(os.listdir(folder)):
    if '.p' in exp:
        ...
        found_adv, rendered_inputs, predictions, images = evaluate_exp(exp_p, render = RENDER_IMAGES, show = False)
...
with open('cma_measured_accuracy_light.txt','a') as F:
