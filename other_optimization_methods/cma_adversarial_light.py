...
def cma_objective_adversarial(x):
    num_lights = int(x.shape[0]/11)
    SCENE_PARAMS[2]['all_light_positions'] = torch.tensor(x[:num_lights*3]).float().view(num_lights,-1)
    SCENE_PARAMS[2]['all_light_look_ats'] = torch.tensor(x[num_lights*3:num_lights*6]).float().view(num_lights,-1)
    SCENE_PARAMS[2]['all_light_intensities'] = torch.tensor(x[num_lights*6:num_lights*9]).float().view(num_lights,-1)
    SCENE_PARAMS[2]['all_light_sizes'] = torch.tensor(x[num_lights*9:]).float().view(num_lights,-1)
...
lp = np.hstack(scene_params[2]['all_light_positions'])
lat = np.hstack(scene_params[2]['all_light_look_ats'])
li = np.hstack(scene_params[2]['all_light_intensities'])
ls = np.hstack(scene_params[2]['all_light_sizes'])
start_pos = np.hstack([lp,lat,li,ls])
